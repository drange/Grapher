...
public class GraphGenerator {
	...
	public static Collection<HashSet<Integer>> extendP5(TestGraph g){
		Collection<HashSet<Integer>> exts = new HashSet<HashSet<Integer>>();
		int n = g.vertexSet().size();
		
		for(int i = 1; i<n; i++){
			NChooseKIterator<Integer> nk = new NChooseKIterator<Integer>(g.vertexSet(), i);
			while(nk.hasNext()){
				Collection<Integer> neigh = nk.next();
				g.addVertex(n);
				for(Integer k : neigh){
					g.addEdge(n, k);
				}
				if(isP5Free(g))
					exts.add(new HashSet<Integer>(neigh));
				g.removeVertex(n);
			}
		}
		return exts;
