...
public class Chordalization<V, E> extends Algorithm<V, E, Set<E>> {
	...
	/**
	 * Minimum fill in on the graph. (Adds the minimum amount of edges such that the graph becomes chordal).
	 * 
	 ...
	 */
	public <V, E> Set<E> fillIn(SimpleGraph<V, E> graph){
		int i = 1;
	 ...
	 * 
	 * @param graph 
	 * @param k Number of edges allowed to add, this affects runtime ~O*(4^k)
	 * @return Set of up to k edges that makes the graph chordal. Null if graph is already chordal,
	 * 			or if the graph can't be made chordal by adding up to k edges.
	 */
	public <V, E> Set<E> chordalize(SimpleGraph<V, E> graph, int k, HashMap<V, Integer> vertexToInt, ArrayList<V> intToVertex ){
	 ...
	 * 
	 * @param graph
	 * @param k Number of edges left to add
	 * @param vertexToInt HashMap to translate vertices into integers, practical for handling list references.
	 * @param intToVertex ArrayList filled with vertices according to their index. 
	 * @return A graph, if there is a chordal graph down the branch. Null if none is found.
	 */
	public <V, E> SimpleGraph<V, E> branching(SimpleGraph<V, E> graph, int k, HashMap<V, Integer> vertexToInt, ArrayList<V> intToVertex){
	...
	/**
	 * Fills an unchorded cycle with edges. There is a bijection between minimum fill-ins of
	 * a cycle of size n and full binary trees with n-2 internal nodes.
	 *  
	 * @param graph
	 * @param cycle An unchorded cycle that was found in the graph 
	 * @param binaryTree A binaryTree representation that corrolates to an unique fill of the cycle.
	 */
	private <V, E> void fillCycle(SimpleGraph<V, E> graph, ArrayList<E> cycle, ArrayList<Boolean> binaryTree){
	 ...
	 * 
	 * @param x The current representation of a binary-tree.
	 * @param n	Number of internal nodes. 
	 * @param ones Count of number of internal nodes used so far.
	 * @param zeros Count of number of external nodes used so far.
	 * @param binaryTrees List of all generated binaryTrees with n internal nodes.
	 */
	 ...
	 * 
	 * @param maxViol List of the three vertices which makes the max violating triple
	 * @param graph
	 * @param vertexToInt
	 * @param intToVertex
	 * @return Unchorded cycle in graph 
	 */
	public <V, E> ArrayList<E> findUnchordedCycle(ArrayList<V> maxViol, SimpleGraph<V, E> graph, HashMap<V, Integer> vertexToInt, ArrayList<V> intToVertex){
		V x;
		V y = null;
	 ...
	 * - Tarjan, Robert E and Yannakakis, Mihalis 1985
	 * 
	 * @param u First vertex of the max violating triple
	 * @param alpha The alpha-values made in the graph
	 * @param graph
	 * @param vertexToInt
	 * @param intToVertex
	 * @return The three vertices of the max violating triple
	 */
	public <V, E> ArrayList<V> maxViolatingTriple(V u, int[][] alpha, SimpleGraph<V, E> graph, HashMap<V, Integer> vertexToInt, ArrayList<V> intToVertex) {
	 ...
	 */
	public <V, E> int[][] maximumCardinalitySearch(SimpleGraph<V, E> graph, HashMap<V, Integer> vertexToInt){
		int i,j;
		int n = graph.vertexSet().size();
		int[] size = new int[n+1];
		int[][] alpha = new int[2][n+1]; // alpha[0][] corresponds to alpha, alpha[1][] corresponds to alphaInverse
	 ...
	 * 
	 * @param graph
	 * @param alpha Alpha-values found by maximum cardinality search.
	 * @param vertexToInt
	 * @param intToVertex
	 * @return The first vertex of the max violating triple if the graph is not chordal. Null if the graph is chordal.
	 */
	public <V, E> V testForZeroFillIn(SimpleGraph<V, E> graph, int[][] alpha, HashMap<V, Integer> vertexToInt, ArrayList<V> intToVertex){
		int w;
		V v;
		int n = graph.vertexSet().size();
		int[] f = new int[n+1];
		...
		for(int i = 1; i <= n; i++){
			...
			for(E e : graph.edgesOf(intToVertex.get(w))){
				if((alpha[0][vertexToInt.get(graph.getEdgeTarget(e))] < i) || (alpha[0][vertexToInt.get(graph.getEdgeSource(e))] < i)){
